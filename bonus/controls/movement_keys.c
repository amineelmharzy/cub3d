...
/*   Updated: 2023/07/25 15:29:04 by mkhellou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	w_key(mlx_key_data_t keydata, t_cub *cub)
{
	...
	if (mlx_is_key_down(cub->mlx, MLX_KEY_W))
	{
		if (!is_in_wall(cub, cub->px + pxo + cub->dx, cub->py)
			&& !is_in_close(cub, cub->px + pxo + cub->dx, cub->py))
			cub->px += cub->dx;
		if (!is_in_wall(cub, cub->px, cub->py + cub->dy + pyo)
			&& !is_in_close(cub, cub->px, cub->py + cub->dy + pyo))
			cub->py += cub->dy;
	}
}
...
void	d_key(mlx_key_data_t keydata, t_cub *cub)
{
	...
	if (mlx_is_key_down(cub->mlx, MLX_KEY_D))
	{
		if (!is_in_wall(cub, cub->px - pxo - cub->dx2, cub->py)
			&& !is_in_close(cub, cub->px - pxo - cub->dx2, cub->py))
			cub->px -= cub->dx2;
		if (!is_in_wall(cub, cub->px, cub->py - cub->dy2 - pyo)
			&& !is_in_close(cub, cub->px, cub->py - cub->dy2 - pyo))
			cub->py -= cub->dy2;
	}
}
